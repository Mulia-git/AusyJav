...
public class DlgPembelian extends javax.swing.JDialog {
    ...
    private double hargakonversi=0,ttl=0,y=0,w=0,ttldisk=0,sbttl=0,ppn=0,jmlkonversi=0,hargappn=0,hargadiskon=0;
    private int jml=0,i=0,row=0,index=0;
    private WarnaTable2 warna=new WarnaTable2();
    private String aktifkanbatch="no",pengaturanharga="",hargadasar="",Pengadaan_Obat=Sequel.cariIsi("select set_akun.Pengadaan_Obat from set_akun"),
            PPN_Masukan=Sequel.cariIsi("select set_akun.PPN_Masukan from set_akun"),kode_akun_bayar="",pakaippn="";
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Transaksi Pengadaan Obat, Alkes & BHP Medis ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        AkunBayar.setBounds(79, 70, 240, 23);
            ...
            if (reply == JOptionPane.YES_OPTION) {
                try {
                    ...
                    if(Sequel.menyimpantf2("pembelian","?,?,?,?,?,?,?,?,?,?,?","No.Faktur",11,new String[]{
                            ...
                            ""+sbttl,""+ttldisk,""+ttl,""+ppn,""+(ttl+ppn),kdgudang.getText(),kode_akun_bayar
                        })==true){
                        jml=tbDokter.getRowCount();
                        for(i=0;i<jml;i++){  
                            setKonversi(i);
                            try {
                                if(Valid.SetAngka(tbDokter.getValueAt(i,0).toString())>0){
                                    if(Sequel.menyimpantf2("detailbeli","?,?,?,?,?,?,?,?,?,?,?,?","Transaksi Pembelian",12,new String[]{
                                        ...
                                        if(aktifkanbatch.equals("yes")){
                                            Trackobat.catatRiwayat(tbDokter.getValueAt(i,2).toString(),Valid.SetAngka(tbDokter.getValueAt(i,12).toString()),0,"Pengadaan",akses.getkode(),kdgudang.getText(),"Simpan",tbDokter.getValueAt(i,13).toString(),NoFaktur.getText(),NoFaktur.getText()+" "+nmsup.getText());
                                            ...
                                            Trackobat.catatRiwayat(tbDokter.getValueAt(i,2).toString(),Valid.SetAngka(tbDokter.getValueAt(i,12).toString()),0,"Pengadaan",akses.getkode(),kdgudang.getText(),"Simpan","","",NoFaktur.getText()+" "+nmsup.getText());
                        ...
                        if(sukses==true){
                            ...
                            if(Sequel.menyimpantf2("tampjurnal","?,?,?,?",4,new String[]{Pengadaan_Obat,"PEMBELIAN",""+ttl,"0"})==false){
                                sukses=false;
                            }
                            if(ppn>0){
                                if(Sequel.menyimpantf2("tampjurnal","?,?,?,?",4,new String[]{PPN_Masukan,"PPN Masukan Obat",""+ppn,"0"})==false){
                                    sukses=false;
                                }
                            }
                            if(Sequel.menyimpantf2("tampjurnal","?,?,?,?",4,new String[]{kode_akun_bayar,AkunBayar.getSelectedItem().toString(),"0",""+(ttl+ppn)})==false){
                                sukses=false;
                            }
                            if(sukses==true){
    ...
    */
    ...
    private void tampil2() {
        try{
            ...
            double[] harga,jumlah,subtotal,diskon,besardiskon,jmltotal,jmlstok,hpp,isi,jmlbesar,ralan,kelas1,kelas2,kelas3,utama,vip,vvip,beliluar,jualbebas,karyawan,dasar;
    ...
    private void simpanbatch(){        
        if((!tbDokter.getValueAt(i,13).toString().equals(""))&&(!tbDokter.getValueAt(i,14).toString().equals(""))&&
            ...
            Sequel.menyimpan2("data_batch","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","Batch",20,new String[]{
                tbDokter.getValueAt(i,13).toString(),tbDokter.getValueAt(i,2).toString(),Valid.SetTgl(TglBeli.getSelectedItem()+""),Valid.SetTgl(tbDokter.getValueAt(i,6).toString()),"Pengadaan",NoFaktur.getText(),
