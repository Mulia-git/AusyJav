...
public final class DlgPermintaanRadiologi extends javax.swing.JDialog {
    ...
    private String kelas_radiologi="Yes",kelas="",cara_bayar_radiologi="Yes",kamar,namakamar,status="",
            norawatibu="",aktifkanparsial="no",finger="",kd_pj="";
    private File file;
...
//        tampil2();
 ...
 private void tampil() {
    ...
    if (this.status.equalsIgnoreCase("Ranap")) {

        if (!kd_pj.equalsIgnoreCase("BPJ")) {

            if (kd_pj.equalsIgnoreCase("UMU")) {
                filterKelas =
                    " ( (jns_perawatan_radiologi.kd_pj='" + kd_pj + "' " +
                    "     AND jns_perawatan_radiologi.kelas='" + kelas + "') " +
                    "   OR (jns_perawatan_radiologi.kd_pj='-' " +
                    "       AND jns_perawatan_radiologi.kelas='-') ) AND ";

            } else {
                 if (kd_pj.equalsIgnoreCase("BPK")) {
                       filterKelas =
                        " jns_perawatan_radiologi.kd_pj='" + kd_pj + "' " +
                        "     AND  jns_perawatan_radiologi.kelas='-'  AND ";
                      }
                          ...
                          filterKelas =
                    " ( (jns_perawatan_radiologi.kd_pj NOT IN ('UMU','BPJ','BPK') " +
                    "     AND jns_perawatan_radiologi.kelas='" + kelas + "') " +
                    "   OR (jns_perawatan_radiologi.kd_pj='-' " +
                    "       AND jns_perawatan_radiologi.kelas='-') ) AND ";
                      }
            ...
            filterKelas =
                " ( jns_perawatan_radiologi.kelas='Rawat Inap' " +
                "   OR (jns_perawatan_radiologi.kd_pj='-' " +
                "       AND jns_perawatan_radiologi.kelas='-') ) AND ";
        }

    } else if (this.status.equalsIgnoreCase("Ralan")) {

         if (kd_pj.equalsIgnoreCase("BPJ") || kd_pj.equalsIgnoreCase("UMU") ) {
                filterKelas =
                    " ( (jns_perawatan_radiologi.kelas='Rawat Jalan' " +
                    "     AND jns_perawatan_radiologi.kd_pj='" + kd_pj + "') " +
                    "   OR (jns_perawatan_radiologi.kd_pj='-' AND jns_perawatan_radiologi.kelas='-') ) AND ";
            } 
            else if (kd_pj.equalsIgnoreCase("BPK")) {
                       filterKelas =
                        " jns_perawatan_radiologi.kd_pj='" + kd_pj + "' " +
                        "     AND jns_perawatan_radiologi.kelas='-' AND " ;
                ...
                filterKelas =
                    " ( (jns_perawatan_radiologi.kelas='Rawat Jalan' " + 
                    "     AND jns_perawatan_radiologi.kd_pj='-') " +
                    "   OR (jns_perawatan_radiologi.kd_pj='-' AND jns_perawatan_radiologi.kelas='-') ) AND ";
            }
    }
System.out.println(filterKelas);




        String cari = TCariPeriksa.getText().trim();
        ...
        try {
            ...
            String sql =
                ...
                "where " + filterKelas +
                "jns_perawatan_radiologi.status='1' ";

           
            if (!cari.equals("")) {
                sql +=
                    "and (jns_perawatan_radiologi.kd_jenis_prw like ? " +
                    "or jns_perawatan_radiologi.nm_perawatan like ?) ";
            }

            sql += "order by jns_perawatan_radiologi.kd_jenis_prw";

            pspemeriksaan = koneksi.prepareStatement(sql);

            if (!cari.equals("")) {
                pspemeriksaan.setString(1, "%" + cari + "%");
                pspemeriksaan.setString(2, "%" + cari + "%");
            }
            ...
            System.out.println("Error tampil pemeriksaan : " + e);
        } finally {
            try {
                if (rs != null) rs.close();
                if (pspemeriksaan != null) pspemeriksaan.close();
            } catch (Exception e) {
    ...
    private boolean isKelasSesuai(JsonNode list) {
        String kelasJson = list.path("Kelas").asText();
        String kdPjJson  = list.path("KodePJ").asText();

        if (kelasJson == null) kelasJson = "";
        if (kdPjJson == null)  kdPjJson  = "";
        ...
        if (status.equalsIgnoreCase("Ranap")) {
            ...
            if (!kd_pj.equalsIgnoreCase("BPJ")) {
                ...
                return kdPjJson.equalsIgnoreCase(kdPjFilter)
                        && kelasJson.equalsIgnoreCase(kelas);
            }

            
            return kelasJson.equalsIgnoreCase("Rawat Inap");
        }

      
        if (status.equalsIgnoreCase("Ralan")) {
            return kelasJson.equalsIgnoreCase("Rawat Jalan");
        }
     ...
     private void tampil2() {
        try {
            ...
            jml = 0;
            for (i = 0; i < tbPemeriksaan.getRowCount(); i++) {
                Object cek = tbPemeriksaan.getValueAt(i, 0);
                if (cek != null && cek.toString().equals("true")) {
                    jml++;
                }
            }
            ...
            String cari = TCariPeriksa.getText().trim().toLowerCase();


            for (JsonNode list : response) {


                if (!isKelasSesuai(list)) {
                    continue;
                }

                String kodePJ  = list.path("KodePJ").asText();
                String kodePrw = list.path("KodePeriksa").asText();
                String namaPrw = list.path("NamaPemeriksaan").asText();

                if (kodePJ == null)  kodePJ = "";
                if (kodePrw == null) kodePrw = "";
                if (namaPrw == null) namaPrw = "";


                // FILTER CARA BAYAR
                if (cara_bayar_radiologi.equals("Yes")) {
                    if (!(kodePJ.equals(Penjab.getText()) || kodePJ.equals("-"))) {
                        continue;
                    }
                }

                if (!cari.equals("")) {
                    if (!kodePrw.toLowerCase().contains(cari) &&
                        !namaPrw.toLowerCase().contains(cari)) {
                        continue;
                    }
                }
            ...
            System.out.println("Notifikasi tampil2 : " + e);
        }
    }
