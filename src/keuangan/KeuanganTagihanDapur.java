 ...
 * @author perpustakaan
 */
public final class KeuanganTagihanDapur extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public KeuanganTagihanDapur(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        if(NoTagihan.getText().trim().equals("")){
            ...
            if (i == JOptionPane.YES_OPTION) {
                ...
                sukses=true;
                if(Sequel.menyimpantf2("dapur_titip_faktur","?,?,?,?,'Ditagihkan'","No.Tagihan",4,new String[]{
                    NoTagihan.getText(),Valid.SetTgl(Tanggal.getSelectedItem()+""),nip.getText(),Keterangan.getText()
                })==true){
                    row=tbBangsal.getRowCount();
                    for(i=0;i<row;i++){
                        if(tabMode.getValueAt(i,0).toString().equals("true")){
                            if(Sequel.menyimpantf2("dapur_detail_titip_faktur","?,?","Detail Tagihan",2,new String[]{
                                NoTagihan.getText(),tbBangsal.getValueAt(i,1).toString(),
                            })==false){
                                sukses=false;
                            }else{
                                Sequel.mengedit("dapurpemesanan","no_faktur=?","status='Titip Faktur'",1,new String[]{tbBangsal.getValueAt(i,1).toString()});
                            }
                        }
                    }
                    bayar=0;
    ...
    private void BtnCari1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCari1ActionPerformed
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        KeuanganCariTagihanDapur form=new KeuanganCariTagihanDapur(null,false);
        form.emptTeks();
        form.isCek();
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            KeuanganTagihanDapur dialog = new KeuanganTagihanDapur(new javax.swing.JFrame(), true);
    ...
    public void tampil(){
        ...
        try{
            ...
            if(ChkTanggalDatang.isSelected()==true){
                tanggaldatang=" dapurpemesanan.tgl_pesan between '"+Valid.SetTgl(TglDatang1.getSelectedItem()+"")+"' and '"+Valid.SetTgl(TglDatang2.getSelectedItem()+"")+"' and ";
            }
            ...
            ps=koneksi.prepareStatement(
                    ...
                    "(SELECT ifnull(SUM(besar_bayar),0) FROM bayar_pemesanan_dapur where bayar_pemesanan_dapur.no_faktur=dapurpemesanan.no_faktur) as bayar, "+
                    "dapursuplier.nama_bank,dapursuplier.rekening from dapurpemesanan inner join dapursuplier inner join petugas "+
                    "on dapurpemesanan.kode_suplier=dapursuplier.kode_suplier "+
                    "and dapurpemesanan.nip=petugas.nip where "+
                    tanggaldatang+tanggaltempo+"(dapurpemesanan.status='Belum Dibayar' or dapurpemesanan.status='Belum Lunas') and dapursuplier.nama_suplier like ? "+
                    (TCari.getText().trim().equals("")?"":"and (dapurpemesanan.no_faktur like ? or dapurpemesanan.no_order like ? or dapurpemesanan.tgl_tempo like ? or "+
                    "dapursuplier.nama_suplier like ? or petugas.nama like ?)")+" order by dapurpemesanan.tgl_tempo ");
            try {
                ...
                try {
                    ...
                    if(rs.next()){
                        if(rs.getString("photo").equals("")||rs.getString("photo").equals("-")){
                            ...
                            LoadHTML.setText("<html><body><center><img src='http://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/penerimaandapur/"+rs.getString("photo")+"' alt='photo' width='"+(internalFrame1.getWidth()-340)+"' height='"+(internalFrame1.getHeight()-275)+"'/></center></body></html>");
                        }  
                    }else{
                        LoadHTML.setText("<html><body><center><br><br><font face='tahoma' size='2' color='#434343'>Kosong</font></center></body></html>");
                    }
    ...
    public void isCek(){
        ...
        if(akses.getjml2()>=1){
            ...
            BtnSimpan.setEnabled(akses.gettitip_faktur_dapur());
