...
public final class DlgAuditKepatuhanAPD extends javax.swing.JDialog {
    ...
    public DlgAuditKepatuhanAPD(java.awt.Frame parent, boolean modal) {
        ...
        setSize(628,674);
        Object[] row={"P","Tanggal Audit","Tindakan","NIP/Kode","Dokter/Paramedis","Profesi/Jabatan","Topi","Masker","Kacamata","Sarung Tangan","Apron","Sepatu","Ttl.Nilai(%)"};
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        for(int i=0;i<tbObat.getRowCount();i++){ 
            if(tbObat.getValueAt(i,0).toString().equals("true")){
                if(Sequel.queryutf("delete from audit_kepatuhan_apd where nik='"+tbObat.getValueAt(i,3).toString()+"' and tindakan='"+tbObat.getValueAt(i,2).toString()+"' "+
            ...
            try {
                ...
                while(rs.next()){
                    ...
                    tabMode.addRow(new Object[]{
                        false,rs.getString("tanggal"),rs.getString("tindakan"),rs.getString("nik"),rs.getString("nama"),rs.getString("jbtn"),rs.getString("topi"),
                ...
                if(i>0){
                    tabMode.addRow(new Object[]{
                        false,"","Ya",":","","",""+ttltopi,""+ttlmasker,""+ttlkacamata,""+ttlsarungtangan,""+ttlapron,""+ttlsepatu,
                        ""+(ttltopi+ttlmasker+ttlkacamata+ttlsarungtangan+ttlapron+ttlsepatu)
                    });
                    tabMode.addRow(new Object[]{
                        false,"","Tidak",":","","",""+(i-ttltopi),""+(i-ttlmasker),""+(i-ttlkacamata),""+(i-ttlsarungtangan),""+(i-ttlapron),""+(i-ttlsepatu),
                        ""+((i-ttltopi)+(i-ttlmasker)+(i-ttlkacamata)+(i-ttlsarungtangan)+(i-ttlapron)+(i-ttlsepatu))
                    });
                    tabMode.addRow(new Object[]{
                        false,"","Rata-rata",":","","",Math.round((ttltopi/i)*100)+" %",Math.round((ttlmasker/i)*100)+" %",Math.round((ttlkacamata/i)*100)+" %",
