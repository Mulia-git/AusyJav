...
public class TokoReturPiutang extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public TokoReturPiutang(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Transaksi Retur Piutang Barang Toko / Minimarket / Koperasi Dari Member ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(50, 50, 50))); // NOI18N
...
private void btnSuplierActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSuplierActionPerformed
        akses.setform("TokoReturPiutang");
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(NoRetur.getText().trim().equals("")){
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                sukses=true;
                if(Sequel.menyimpantf2("tokoreturpiutang","?,?,?,?,?,?","No.Retur",6,new String[]{
                    NoRetur.getText(),Valid.SetTgl(TglRetur.getSelectedItem()+""),kdptg.getText(),kdsup.getText(),Catatan.getText(),ttl+""
                })==true){
                    jml=tbDokter.getRowCount();
                    for(i=0;i<jml;i++){
                        if(Valid.SetAngka(tbDokter.getValueAt(i,0).toString())>0){
                            if(Sequel.menyimpantf2("toko_detail_returpiutang","?,?,?,?,?,?,?,?","Transaksi Retur Piutang",8,new String[]{
                                ...
                                Trackbarang.catatRiwayat(tbDokter.getValueAt(i,1).toString(),Valid.SetAngka(tbDokter.getValueAt(i,0).toString()),0,"Retur Piutang", akses.getkode(),"Simpan");
                ...
                if(sukses==true){
                    ...
                    if(Sequel.menyimpantf2("tampjurnal","?,?,?,?",4,new String[]{Retur_Piutang_Toko,"RETUR PIUTANG",""+ttl,"0"})==false){
                        sukses=false;
                    }
                    if(Sequel.menyimpantf2("tampjurnal","?,?,?,?",4,new String[]{Kontra_Retur_Piutang_Toko,"KONTRA RETUR PIUTANG","0",""+ttl})==false){
                        sukses=false;
                    }
                    if(sukses==true){
    ...
    */
    ...
    public void isCek(){
        ...
        if(akses.getjml2()>=1){
            ...
            BtnSimpan.setEnabled(akses.gettoko_retur_piutang());
    ...
    private void autoNomor() {
        Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(no_retur_piutang,3),signed)),0) from tokoreturpiutang where tgl_retur='"+Valid.SetTgl(TglRetur.getSelectedItem()+"")+"'","TRP"+TglRetur.getSelectedItem().toString().substring(6,10)+TglRetur.getSelectedItem().toString().substring(3,5)+TglRetur.getSelectedItem().toString().substring(0,2),3,NoRetur); 
    }
