...
public class DapurReturBeli extends javax.swing.JDialog {
    ...
    private DapurCariReturBeli form=new DapurCariReturBeli(null,false);
    ...
    private String Retur_Beli_Dapur=Sequel.cariIsi("select set_akun2.Retur_Beli_Dapur from set_akun2"),Kontra_Retur_Beli_Dapur=Sequel.cariIsi("select set_akun2.Kontra_Retur_Beli_Dapur from set_akun2");

    /** Creates new form DlgProgramStudi
     * @param parent
     * @param modal */
    public DapurReturBeli(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        form.suplier.addWindowListener(new WindowListener() {
            ...
            @Override
            public void windowClosed(WindowEvent e) {
                if(akses.getform().equals("DapurReturBeli")){
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(NoRetur.getText().trim().equals("")){
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                sukses=true;
                if(Sequel.menyimpantf2("dapurreturbeli","?,?,?,?,?,?","No.Retur",6,new String[]{
                    NoRetur.getText(),Valid.SetTgl(TglRetur.getSelectedItem()+""),kdptg.getText(),kdsup.getText(),Catatan.getText(),ttl+""
                })==true){
                    jml=tbDokter.getRowCount();
                    for(i=0;i<jml;i++){
                        if(Valid.SetAngka(tbDokter.getValueAt(i,0).toString())>0){
                            if(Sequel.menyimpantf2("dapur_detail_returbeli","?,?,?,?,?,?,?,?","Transaksi Retur Beli",8,new String[]{
                ...
                if(sukses==true){
                    ...
                    if(Sequel.menyimpantf2("tampjurnal","?,?,?,?",4,new String[]{Retur_Beli_Dapur,"RETUR PEMBELIAN","0",""+ttl})==false){
                        sukses=false;
                    }
                    if(Sequel.menyimpantf2("tampjurnal","?,?,?,?",4,new String[]{Kontra_Retur_Beli_Dapur,"KONTRA RETUR PEMBELIAN",""+ttl,"0"})==false){
                        sukses=false;
                    }
                    if(sukses==true){
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DapurReturBeli dialog = new DapurReturBeli(new javax.swing.JFrame(), true);
    ...
    private void tampil() {
        ...
        try{
            ps=koneksi.prepareStatement(
                    "select dapurbarang.kode_brng,dapurbarang.nama_brng,dapurbarang.kode_sat,dapurbarang.harga,dapurbarang.stok "+
                    " from dapurbarang where dapurbarang.status='1' and dapurbarang.kode_brng like ? or "+
            ...
            try{   
                ...
                rs=ps.executeQuery();
                while(rs.next()){
                    tabMode.addRow(new Object[]{"",rs.getString("kode_brng"),rs.getString("nama_brng"),rs.getString("kode_sat"),rs.getDouble("harga"),0,"",rs.getDouble("stok"),rs.getDouble("harga")});
                }   
            }catch(Exception e){
    ...
    public void isCek(){
        ...
        if(akses.getjml2()>=1){
            ...
            BtnSimpan.setEnabled(akses.getdapur_returbeli());
    ...
    private void autoNomor() {
        Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(dapurreturbeli.no_retur_beli,3),signed)),0) from dapurreturbeli where dapurreturbeli.tgl_retur='"+Valid.SetTgl(TglRetur.getSelectedItem()+"")+"'","TRB"+TglRetur.getSelectedItem().toString().substring(6,10)+TglRetur.getSelectedItem().toString().substring(3,5)+TglRetur.getSelectedItem().toString().substring(0,2),3,NoRetur); 
    }
