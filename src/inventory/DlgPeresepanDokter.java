...
public final class DlgPeresepanDokter extends javax.swing.JDialog {
    ...
    private String norawat="",kode_penjab="",kode_bpjs="",kdjns_obat_bpjs="",kode_umum="",kdjns_obat_umum="",kdjns_obat_asuransi="",obatpercarabayar="",caraBayar="",noracik="",aktifkanbatch="no",STOKKOSONGRESEP="no",qrystokkosong="",qry_obat_per_cara_bayar="",inner_obat_per_cara_bayar="",tampilkan_ppnobat_ralan="",status="",bangsal="",resep="",DEPOAKTIFOBAT="",
    ...
    public DlgPeresepanDokter(java.awt.Frame parent, boolean modal) {
        ...
        try {
            obatpercarabayar=Sequel.cariIsi("SELECT per_cara_bayar FROM setting_obat");
        } catch (Exception e) {
            obatpercarabayar="no";
        }
    ...
    private void initComponents() {
        ...
        DTPBeri.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "16-07-2025" }));
...
private void ppBersihkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBersihkanActionPerformed
    ...
    */
    ...
    private void buatcacheresep(){
        ...
        kode_penjab=Sequel.cariIsi("SELECT kd_pj FROM reg_periksa WHERE no_rawat = '"+norawat+"'");
        try{
//            System.out.println("ini adalah no Rawat : "+norawat);
            ...
            if(kenaikan>0){
                if(aktifkanbatch.equals("yes")){
                    ...
                    psresep=koneksi.prepareStatement(
                        ...
                        " (databarang.karyawan+(databarang.karyawan*?)) as karyawan,(databarang.ralan+(databarang.ralan*?)) as ralan,(databarang.beliluar+(databarang.beliluar*?)) as beliluar,(databarang.kelas1+(databarang.kelas1*?)) as kelas1," +
                        " (databarang.kelas2+(databarang.kelas2*?)) as kelas2,(databarang.kelas3+(databarang.kelas3*?)) as kelas3,(databarang.vip+(databarang.vip*?)) as vip,(databarang.vvip+(databarang.vvip*?)) as vvip,"+
                    ...
                    qry_obat_per_cara_bayar="";
                    inner_obat_per_cara_bayar="";
                    if(obatpercarabayar.equals("yes")){
                        inner_obat_per_cara_bayar=" inner join mapping_obat_cara_bayar ON mapping_obat_cara_bayar.kode_brng=databarang.kode_brng ";
                        qry_obat_per_cara_bayar=" and mapping_obat_cara_bayar.kd_pj IN ('"+kode_penjab+"','-') ";
                    }
                    ...
                    psresep=koneksi.prepareStatement(
                        ...
                        " from databarang inner join jenis on databarang.kdjns=jenis.kdjns "+
                        " inner join industrifarmasi on industrifarmasi.kode_industri=databarang.kode_industri "+
                        " inner join gudangbarang on databarang.kode_brng=gudangbarang.kode_brng "+inner_obat_per_cara_bayar+
                        " where  databarang.status='1' "+qry_obat_per_cara_bayar+qrystokkosong+" and gudangbarang.no_batch='' and gudangbarang.no_faktur='' and gudangbarang.kd_bangsal=? "+
                        " order by databarang.nama_brng");
                }
                    
                try{
                    psresep.setDouble(1,kenaikan);
                    psresep.setDouble(2,kenaikan);
                    psresep.setDouble(3,kenaikan);
                    psresep.setDouble(4,kenaikan);
                    psresep.setDouble(5,kenaikan);
                    psresep.setDouble(6,kenaikan);
                    psresep.setDouble(7,kenaikan);
                    psresep.setDouble(8,kenaikan);
                    psresep.setString(9,bangsal);
                    rsobat=psresep.executeQuery();
                    while(rsobat.next()){
            ...
            if(kenaikan>0){
                if(aktifkanbatch.equals("yes")){
                    ...
                    psresep=koneksi.prepareStatement(
                        ...
                        " inner join industrifarmasi on industrifarmasi.kode_industri=databarang.kode_industri "+
                        " inner join gudangbarang on databarang.kode_brng=gudangbarang.kode_brng "+
                        " inner join resep_dokter on resep_dokter.kode_brng=databarang.kode_brng "+inner_obat_per_cara_bayar+
                        " where  databarang.status='1' "+qry_obat_per_cara_bayar+qrystokkosong+" and gudangbarang.no_batch='' and gudangbarang.no_faktur='' and gudangbarang.kd_bangsal=? and "+
                    ...
                    if(kenaikan>0){
                        if(aktifkanbatch.equals("yes")){
                            ...
                            ps2=koneksi.prepareStatement(
                                ...
                                "inner join industrifarmasi on industrifarmasi.kode_industri=databarang.kode_industri "+
                                "inner join gudangbarang on databarang.kode_brng=gudangbarang.kode_brng "+
                                "inner join resep_dokter_racikan_detail on resep_dokter_racikan_detail.kode_brng=databarang.kode_brng "+inner_obat_per_cara_bayar+
                                "where databarang.status='1' "+qry_obat_per_cara_bayar+qrystokkosong+" and gudangbarang.no_batch='' and gudangbarang.no_faktur='' and gudangbarang.kd_bangsal=? and "+
    ...
    private void SetHarga() {
        if(status.equals("ranap")){
            ...
            caraBayar = Sequel.cariIsi("select reg_periksa.kd_pj from reg_periksa where reg_periksa.no_rawat=?",TNoRw.getText());
