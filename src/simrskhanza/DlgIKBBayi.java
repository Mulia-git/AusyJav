...
public class DlgIKBBayi extends javax.swing.JDialog {
    ...
    private String pengurutan="",bulan="",tahun="",awalantahun="",awalanbulan="",posisitahun="",
            ...
            krjs2="",umribu="",umrayah="",umrplpr="",umrs1="",umrs2="",no_rm="";
...
private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        if(NmBayi.getText().trim().equals("")){
            ...
            if(Sequel.menyimpantf("pasien","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Rekam Medis Pasien",36,new String[]{
                no_rm,NmBayi.getText(),"-",JKel.getSelectedItem().toString().substring(0,1),"-",Valid.SetTgl(Lahir.getSelectedItem()+""),Nmibu.getText(),AlamatIbu.getText(),"-","-","BELUM MENIKAH","-",
                ...
                try {
                    if(Sequel.menyimpantf("pasien_bayi","'"+no_rm+"','"+
                        ...
                        Mikasi.getText()+"','"+Mikonium.getText()+"'","No.RM/No.SKL")==true){
                            ...
                            Sequel.queryu2("insert into set_no_rkm_medis values(?)",1,new String[]{no_rm}); 
                            emptTeks();
                    }else{
                    ...
                    if(Sequel.menyimpantf("pasien","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Rekam Medis Pasien",36,new String[]{
                        ...
                        try {
                            if(Sequel.menyimpantf("pasien_bayi","'"+no_rm+"','"+
                                ...
                                Sequel.meghapus("pasien","no_rkm_medis",no_rm);
                            }  
                        } catch (Exception e) {
                            Sequel.meghapus("pasien","no_rkm_medis",no_rm);
                            JOptionPane.showMessageDialog(null,"Gagal menyimpan kelahiran bayi, silahkan periksa data yang mau disimpan..!!");
...
private void MnKartuActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnKartuActionPerformed
    ...
    */
    ...
    private String auto_norm(){
        
        String no_rm = Sequel.cariIsi("SELECT (no_rkm_medis + 1) as no_rkm_medis FROM set_no_rkm_medis");
        
        int no_rm_terakhir = Integer.parseInt(Sequel.cariIsi("SELECT (no_rkm_medis + 1) as no_rkm_medis FROM set_no_rkm_medis"));
        int no_rm_lain = Integer.parseInt(NoRm.getText());
        
        if(no_rm_terakhir > no_rm_lain){
            no_rm = no_rm;
        }else{
            no_rm = NoRm.getText();
        }
