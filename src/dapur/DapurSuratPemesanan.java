...
public class DapurSuratPemesanan extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public DapurSuratPemesanan(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        BtnCari1.setBounds(462, 65, 28, 23);
        ...
        BtnTambah.setBounds(522, 65, 28, 23);
...
private void btnPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPetugasActionPerformed
    ...
    private void BtnCariActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCariActionPerformed
        ...
        DapurCariSuratPemesanan form=new DapurCariSuratPemesanan(null,false);
        form.emptTeks();
        form.isCek();
        ...
        if(tbDokter.getRowCount()!=0){
            if(evt.getKeyCode()==KeyEvent.VK_ENTER){
                ...
                try {
                    ...
                    }else if(tbDokter.getSelectedColumn()==4){
                ...
                try {
                    if(tbDokter.getSelectedColumn()==0){
                        ...
                        if(tbDokter.getValueAt(tbDokter.getSelectedRow(),4).toString().equals("0")||tbDokter.getValueAt(tbDokter.getSelectedRow(),7).toString().equals("0.0")||tbDokter.getValueAt(tbDokter.getSelectedRow(),7).toString().equals("0,0")){
                            tbDokter.setValueAt(0, tbDokter.getSelectedRow(),4);
                        }
                    }else if(tbDokter.getSelectedColumn()==6){
                        if(tbDokter.getValueAt(tbDokter.getSelectedRow(),6).toString().equals("0")||tbDokter.getValueAt(tbDokter.getSelectedRow(),9).toString().equals("0.0")||tbDokter.getValueAt(tbDokter.getSelectedRow(),9).toString().equals("0,0")){
                            tbDokter.setValueAt(0, tbDokter.getSelectedRow(),6);
                        }
                    }else if(tbDokter.getSelectedColumn()==7){
                        if(tbDokter.getValueAt(tbDokter.getSelectedRow(),7).toString().equals("0")||tbDokter.getValueAt(tbDokter.getSelectedRow(),10).toString().equals("0.0")||tbDokter.getValueAt(tbDokter.getSelectedRow(),10).toString().equals("0,0")){
                            tbDokter.setValueAt(0, tbDokter.getSelectedRow(),7);
                        }
                    }
                } catch (Exception e) {                    
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(NoPemesanan.getText().trim().equals("")){
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                sukses=true;
                if(Sequel.menyimpantf2("surat_pemesanan_dapur","?,?,?,?,?,?,?,?,?,?,?","No.Pemesanan",11,new String[]{
                    ...
                    for(i=0;i<jml;i++){
                        if(Valid.SetAngka(tbDokter.getValueAt(i,0).toString())>0){
                            if(Sequel.menyimpantf2("detail_surat_pemesanan_dapur","?,?,?,?,?,?,?,?,?","Transaksi Pemesanan",9,new String[]{
                                NoPemesanan.getText(),
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DapurSuratPemesanan dialog = new DapurSuratPemesanan(new javax.swing.JFrame(), true);
    ...
    private void tampil() {
        try{
            ...
            file=new File("./cache/suratpemesanandapur.iyem");
            file.createNewFile();
            fileWriter = new FileWriter(file);
            StringBuilder iyembuilder = new StringBuilder();
            
            ps=koneksi.prepareStatement("select dapurbarang.kode_brng, dapurbarang.nama_brng,dapurbarang.kode_sat, "+
            ...
            if (iyembuilder.length() > 0) {
                ...
                fileWriter.write("{\"suratpemesanandapur\":["+iyembuilder+"]}");
                fileWriter.flush();
            }
    ...
    private void tampil2() {
        try{
            ...
            myObj = new FileReader("./cache/suratpemesanandapur.iyem");
            root = mapper.readTree(myObj);
            response = root.path("suratpemesanandapur");
            if(response.isArray()){
    ...
    private void autoNomor() {
        Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(surat_pemesanan_dapur.no_pemesanan,3),signed)),0) from surat_pemesanan_dapur where surat_pemesanan_dapur.tanggal='"+Valid.SetTgl(Tanggal.getSelectedItem()+"")+"' ",
                "SPD"+Tanggal.getSelectedItem().toString().substring(6,10)+Tanggal.getSelectedItem().toString().substring(3,5)+Tanggal.getSelectedItem().toString().substring(0,2),3,NoPemesanan); 
    }
