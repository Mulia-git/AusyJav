...
public class DapurPengajuan extends javax.swing.JDialog {
    ...
    private String[] jumlah,kodebarang,namabarang,satuan,dapurjenisbarang;
     ...
     * @param parent
     * @param modal */
    public DapurPengajuan(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();

        Object[] judul={"Jml","Kode Barang","Nama Barang","Satuan","Jenis Barang","Harga","Sub Total"};
    ...
    private void initComponents() {
        ...
        TCari.setPreferredSize(new java.awt.Dimension(145, 23));
    ...
    private void BtnCariActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCariActionPerformed
        ...
        DapurCariPengajuan form=new DapurCariPengajuan(null,false);
        form.emptTeks(); 
        form.isCek();
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        if(NoPengajuan.getText().trim().equals("")){
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                sukses=true;
                if(Sequel.menyimpantf2("pengajuan_barang_dapur","?,?,?,?,?","No.Pengajuan",5,new String[]{
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DapurPengajuan dialog = new DapurPengajuan(new javax.swing.JFrame(), true);
    ...
    private void tampil() {
        try{
            ...
            file=new File("./cache/pengajuandapur.iyem");
            file.createNewFile();
            ...
            if (iyembuilder.length() > 0) {
                ...
                fileWriter.write("{\"pengajuandapur\":["+iyembuilder+"]}");
                fileWriter.flush();
            }
    ...
    private void tampil2() {
        try{
            ...
            dapurjenisbarang=new String[jml];
            ...
            for(i=0;i<row;i++){
                if(!tbDokter.getValueAt(i,0).toString().equals("")){
                    ...
                    dapurjenisbarang[index]=tbDokter.getValueAt(i,4).toString();
            ...
            Valid.tabelKosong(tabMode);
            for(i=0;i<jml;i++){
                tabMode.addRow(new Object[]{jumlah[i],kodebarang[i],namabarang[i],satuan[i],dapurjenisbarang[i],harga[i],subtotal[i]});
            }
            ...
            satuan=null;
            jumlah=null;
            dapurjenisbarang=null;
            harga=null;
            subtotal=null;
            
            
            myObj = new FileReader("./cache/pengajuandapur.iyem");
            root = mapper.readTree(myObj);
            response = root.path("pengajuandapur");
            if(response.isArray()){
                if(TCari.getText().trim().equals("")){
                    for(JsonNode list:response){
                        tabMode.addRow(new Object[]{
                            "",list.path("KodeBarang").asText(),list.path("NamaBarang").asText(),list.path("Satuan").asText(),list.path("Jenis").asText(),list.path("Harga").asDouble(),0
                        });
                    }
                }else{
                    for(JsonNode list:response){
    ...
    public void isCek(){
        ...
        if(akses.getjml2()>=1){
            ...
            BtnSimpan.setEnabled(akses.getpengajuan_barang_dapur());
    ...
    private void autoNomor() {
        Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(pengajuan_barang_dapur.no_pengajuan,3),signed)),0) from pengajuan_barang_dapur where pengajuan_barang_dapur.tanggal='"+Valid.SetTgl(Tanggal.getSelectedItem()+"")+"' ",
                "PBD"+Tanggal.getSelectedItem().toString().substring(6,10)+Tanggal.getSelectedItem().toString().substring(3,5)+Tanggal.getSelectedItem().toString().substring(0,2),3,NoPengajuan); 
    }
    ...
    private void getData(){        
        ...
        for(i=0;i<jml;i++){    
            ...
            if(Valid.SetAngka(tbDokter.getValueAt(i,0).toString())>0){  
                try {
                    y=Double.parseDouble(tbDokter.getValueAt(i,6).toString());
                } catch (Exception e) {
