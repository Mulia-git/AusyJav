 ...
 * @author perpustakaan
 */
public final class KeuanganTagihanNonMedis extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public KeuanganTagihanNonMedis(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Titip Faktur/Tagihan Barang Non Medis dan Penunjang ( Lab & RO ) ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(50, 50, 50))); // NOI18N
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        if(NoTagihan.getText().trim().equals("")){
            ...
            if (i == JOptionPane.YES_OPTION) {
                ...
                sukses=true;
                if(Sequel.menyimpantf2("ipsrs_titip_faktur","?,?,?,?,'Ditagihkan'","No.Tagihan",4,new String[]{
                    NoTagihan.getText(),Valid.SetTgl(Tanggal.getSelectedItem()+""),nip.getText(),Keterangan.getText()
                })==true){
                    row=tbBangsal.getRowCount();
                    for(i=0;i<row;i++){
                        if(tabMode.getValueAt(i,0).toString().equals("true")){
                            if(Sequel.menyimpantf2("ipsrs_detail_titip_faktur","?,?","Detail Tagihan",2,new String[]{
                                NoTagihan.getText(),tbBangsal.getValueAt(i,1).toString(),
                            })==false){
                                sukses=false;
                            }else{
                                Sequel.mengedit("ipsrspemesanan","no_faktur=?","status='Titip Faktur'",1,new String[]{tbBangsal.getValueAt(i,1).toString()});
                            }
                        }
                    }
                    bayar=0;
    ...
    private void BtnCari1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCari1ActionPerformed
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        KeuanganCariTagihanNonMedis form=new KeuanganCariTagihanNonMedis(null,false);
        form.emptTeks();
        form.isCek();
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            KeuanganTagihanNonMedis dialog = new KeuanganTagihanNonMedis(new javax.swing.JFrame(), true);
    ...
    private void autoNomor() {
        Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(no_tagihan,3),signed)),0) from ipsrs_titip_faktur where tanggal='"+Valid.SetTgl(Tanggal.getSelectedItem()+"")+"' ",
